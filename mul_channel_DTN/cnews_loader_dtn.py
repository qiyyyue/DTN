...
def rcnn_batch_iter(kg_x, kg_y, w2v_x, y, batch_size=32):
    """生成批次数据"""
    data_len = len(kg_x)
    num_batch = int((data_len - 1) / batch_size) + 1
    ...
    for i in range(num_batch):
        start_id = i * batch_size
        end_id = min((i + 1) * batch_size, data_len)
        yield kg_x_shuffle[start_id:end_id], kg_y_shuffle[start_id:end_id], w2v_x_shuffle[start_id:end_id], y_shuffle[start_id:end_id]
...
def rcnn_file_process(filename, kg_x, kg_y, w2v, cat_to_id, max_length=30):
    """将文件转换为id表示"""
    contents, labels = read_file_tmp(filename)
    kg_matrix_x, kg_matrix_y, w2v_matrix, label_id = [], [], [], []
    # print(len(contents))
    for i in range(len(contents)):
        kg_matrix_x.append(triple_kg_embd(contents[i], kg_x))
        kg_matrix_y.append(triple_kg_embd(contents[i], kg_y))
        w2v_matrix.append([w2v[x] for x in contents[i] if x in w2v])
        label_id.append(cat_to_id[labels[i]])
        print(str(i))

    # 使用keras提供的pad_sequences来将文本pad为固定长度
    kg_x_pad = kr.preprocessing.sequence.pad_sequences(kg_matrix_x, max_length)
    kg_y_pad = kr.preprocessing.sequence.pad_sequences(kg_matrix_y, max_length)
    w2v_x_pad = kr.preprocessing.sequence.pad_sequences(w2v_matrix, max_length)
    y_pad = kr.utils.to_categorical(label_id, num_classes=len(cat_to_id))  # 将标签转换为one-hot表示

    return kg_x_pad, kg_y_pad, w2v_x_pad, y_pad