...
class DTN(object):
    ...
    def __init__(self, config):
        ...
        self.kg_input_y = tf.placeholder(tf.float32, [None, self.config.seq_length, self.config.kg_embedding_dim], name='kg_input_x')
        self.cnn_input_x = tf.placeholder(tf.float32, [None, None, None, 64], name='cnn_input_x')
    ...
    def rcnn(self):
        ...
        with tf.name_scope("gru_rnn"):
            #gru rnn net work
            gru_cells = [kg_dropout() for _ in range(self.config.num_layers)]
            gru_rnn_cell = tf.contrib.rnn.MultiRNNCell(gru_cells)

        with tf.name_scope("kg_rnn_x"):
            # 多层rnn网络
            _kg_outputs_x, _ = tf.nn.dynamic_rnn(cell=gru_rnn_cell, inputs=self.kg_input_x, dtype=tf.float32)
            kg_last_x = _kg_outputs_x[:, -1, :]  # 取最后一个时序输出作为结果

        with tf.name_scope("kg_rnn_y"):
            # 多层rnn网络
            _kg_outputs_y, _ = tf.nn.dynamic_rnn(cell=gru_rnn_cell, inputs=self.kg_input_x, dtype=tf.float32)
            kg_last_y = _kg_outputs_y[:, -1, :]  # 取最后一个时序输出作为结果

        with tf.name_scope("kg_score_x"):
            # 全连接层，后面接dropout以及relu激活
            rnn_fc_x = tf.layers.dense(kg_last_x, self.config.rnn_hidden_dim, name='fc_kg_x')
            rnn_fc_x = tf.contrib.layers.dropout(rnn_fc_x, self.keep_prob)
            rnn_fc_x = tf.nn.relu(rnn_fc_x)
            self.rnn_fc_x = rnn_fc_x

        with tf.name_scope("kg_score_y"):
            rnn_fc_y = tf.layers.dense(kg_last_y, self.config.rnn_hidden_dim, name='fc_kg_y')
            rnn_fc_y = tf.contrib.layers.dropout(rnn_fc_y, self.keep_prob)
            rnn_fc_y = tf.nn.relu(rnn_fc_y)
            self.rnn_fc_y = rnn_fc_y
        ...
        with tf.name_scope("cnn"):
            ...
            conv = tf.layers.conv1d(tf.to_float(tf.stack([rnn_fc_x, rnn_fc_y, w2v_rnn_fc], 1)), self.config.num_filters, self.config.kernel_size, name='conv')
