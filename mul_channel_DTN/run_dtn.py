...
from mul_channel_DTN.dtn_model import DTN, DTNConfig
from mul_channel_DTN.cnews_loader_dtn import word_to_vec, read_category, rcnn_batch_iter, rcnn_file_process
...
def feed_data(kg_x_batch, kg_y_batch, w2v_x_batch, y_batch, keep_prob):
    feed_dict = {
        model.kg_input_x: kg_x_batch,
        model.kg_input_y: kg_y_batch,
...
def evaluate(sess, kg_x_, kg_y_, w2v_x_, y_):
    """评估在某一数据上的准确率和损失"""
    data_len = len(kg_x_)
    batch_eval = rcnn_batch_iter(kg_x_, kg_y_, w2v_x_, y_, config.batch_size)
    total_loss = 0.0
    total_acc = 0.0
    for kg_x_batch, kg_y_batch, w2v_batch, y_batch in batch_eval:
        batch_len = len(kg_x_batch)
        feed_dict = feed_data(kg_x_batch, kg_y_batch, w2v_batch, y_batch, 1.0)
...
def train():
    ...
    tensorboard_dir = '../tensorboard/dtn/dtn'
    ...
    kg_x_train, kg_y_train, w2v_x_train, y_train = rcnn_file_process(train_dir, t2v_x, t2v_y, w2v, cat_to_id, config.seq_length)
    kg_x_val, kg_y_val, w2v_x_val, y_val = rcnn_file_process(val_dir, t2v_x, t2v_y, w2v, cat_to_id, config.seq_length)
    ...
    for epoch in range(config.num_epochs):
        print('Epoch:', epoch + 1)
        batch_train = rcnn_batch_iter(kg_x_train, kg_y_train, w2v_x_train, y_train, config.batch_size)
        for kg_x_batch, kg_y_batch, w2v_x_batch, y_batch in batch_train:
            feed_dict = feed_data(kg_x_batch, kg_y_batch, w2v_x_batch, y_batch, config.dropout_keep_prob)
            ...
            if total_batch % config.print_per_batch == 0:
                ...
                loss_val, acc_val = evaluate(session, kg_x_val, kg_y_val, w2v_x_val, y_val)  # todo
                # fc = model.fc
                # print(fc)
...
def test():
    ...
    start_time = time.time()
    kg_x_test, kg_y_test, w2v_x_test, y_test = rcnn_file_process(test_dir, t2v_x, t2v_y, w2v, cat_to_id, config.seq_length)
    ...
    print('Testing...')
    loss_test, acc_test = evaluate(session, kg_x_test, kg_y_test, w2v_x_test, y_test)
    ...
    for i in range(num_batch):  # 逐批次处理
        ...
        feed_dict = {
            ...
            model.kg_input_y: kg_x_test[start_id:end_id],
...
if __name__ == '__main__':
    ...
    t2v_x = triple2vec(20, "../model/kg_embedding_model/TML1K/")
    t2v_y = triple2vec(20, "../model/kg_embedding_model/dbpedia_model/e_tr/")

    print(len(t2v_x.entity2id_dict))
    print(len(t2v_y.entity2id_dict))
